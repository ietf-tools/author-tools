...
def get_text_id_from_file(file, upload_dir, raw=False, text_or_xml=False,
                          logger=getLogger()):
    ...
    if not raw:
        ...
        if not text_or_xml or (
                text_or_xml and file_ext not in ['.txt', '.xml']):
            (dir_path, filename) = get_text_id(dir_path, filename, logger)

    return (dir_path, filename)


def get_text_id_from_url(url, upload_dir, raw=False, text_or_xml=False,
                         logger=getLogger()):
