...
class TestApiidnits3(TestCase):
    ...
    def test_invalid_url(self):
        ...
        with self.app.test_client() as client:
            with self.app.app_context():
                result = client.get("/api/idnits3?" + urlencode({"url": url}))
                json_data = result.get_json()
    ...
    def test_idnits3_options(self):
        url = "https://www.ietf.org/archive/id/draft-iab-xml2rfcv2-02.txt"
        id = "draft-iab-xml2rfcv2-02"
        params = {"url": url, "year": 2015}

        with self.app.test_client() as client:
            with self.app.app_context():
                result = client.get("/api/idnits3?" + urlencode(params))
                data = result.get_data(as_text=True)
    ...
    def test_kramdown_error_url(self):
        ...
        responses.add(responses.GET, url, body="{::boilerplate foobar}", status=200)
        with self.app.test_client() as client:
            with self.app.app_context():
                result = client.get("/api/idnits3?" + urlencode({"url": url}))
                json_data = result.get_json()
        ...
        with self.app.test_client() as client:
            with self.app.app_context():
                result = client.post(
                    "/api/idnits3?submission=True",
                    data={
                        "file": (open(get_path(TEST_TEXT_ERROR), "rb"), TEST_TEXT_ERROR)
                    },
                )
                data = result.get_data(as_text=True)
