...
class TestUtilsAuthentication(TestCase):
    ...
    def test_authentication_missing_api_key(self):
        responses.add(responses.POST, DT_APPAUTH_URL, status=400)

        with self.app.test_client() as client:
            with self.app.app_context():
                filename = get_path(TEST_XML_DRAFT)
                result = client.post(
                    "/api/render/xml", data={"file": (open(filename, "rb"), filename)}
                )
    ...
    def test_authentication_valid_api_key(self):
        responses.add(
            responses.POST, DT_APPAUTH_URL, json={"success": True}, status=200
        )
    ...
    def test_authentication_invalid_api_key(self, api_key):
        responses.add(responses.POST, DT_APPAUTH_URL, status=403)
